...
public class Lota : MonoBehaviour {
	...
	void OnCollisionEnter (Collision col) {
        if(col.gameObject.name == "Port1") {
           cf.force = new Vector3(Random.Range(-3.0f, 3.0f),0.0f,Random.Range(-6.0f,-3.0f));
           ausour.clip = audios[0];
        }
        else if(col.gameObject.name == "Port2") {
        	cf.force = new Vector3(Random.Range(-6.0f,-3.0f),0.0f,Random.Range(-3.0f, 3.0f)); ausour.clip = audios[0];
        }
        else if(col.gameObject.name == "Port") {
            cf.force = new Vector3(Random.Range(3.0f, 6.0f),0.0f,Random.Range(-3.0f, 3.0f)); ausour.clip = audios[0];
        }
        else if(col.gameObject.name == "Player") {
            cf.force = new Vector3(Random.Range(-3.0f, 3.0f),0.0f,Random.Range(3.0f, 6.0f)); ausour.clip = audios[0];
        } 
        else if(col.gameObject.name == "PlayerPort"){
        	tr.transform.position = new Vector3(0.0f, 0.0f, -5.0f); ausour.clip = audios[1];
        	cf.force = new Vector3(Random.Range(-6.0f, 6.0f),0.0f,0.0f);
